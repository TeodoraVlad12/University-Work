...
void testNew() {
    ...
    cout << "Finished testing the new function! :) " << endl;
    list.removeBetween(2, 4);            //removing a single element
    assert(list.size() == 5);
    assert(list.search(4) == -1);
    //list.removeBetween(0, 5);
    //assert(list.size() == 2);

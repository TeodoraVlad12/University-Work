...
public class MyHeap<V> implements MyIHeap<V>{
    ...
    public synchronized int addNewHeapEntry(V value) {
        ...
        return this.nextFreeLocation - 1;   // returns the address on which the model.value was stored
    }

    @Override
    public synchronized V getHeapValue(int address) throws MyException {
        if (!this.elems.containsKey(address)) {
            throw new MyException("ERROR: Could not return heap model.value. The given address is not a key in the heap table.");
        }
        else {
